"""
Emotional AI – Script 3: Train HuBERT-Large  (v2 — OOM + speed fixes)
======================================================================
...
Fixes vs v1:
  1. OOM: num_workers reduced, persistent_workers, prefetch_factor tuned
  2. OOM: speed-perturbation augmentation removed from worker processes
          (it was allocating a new Resample kernel per sample → RAM spike)
          Replaced with lightweight waveform-only ops safe for workers
  3. Speed: pin_memory=True only when CUDA available
  4. Speed: torch.compile() on classifier head (optional, auto-disabled if unsupported)
  5. Speed: gradient checkpointing on HuBERT encoder to cut VRAM → larger effective batch
  6. Speed: bf16 mixed precision handled by accelerate (pass --mixed_precision=bf16)
  7. Checkpoint: plain state_dict save (no accelerate bundle)
  8. Focal loss + label smoothing kept
  9. Added per-epoch ETA estimate

Launch (single GPU):
    python script3_train.py

Launch (multi-GPU, recommended):
    accelerate launch --num_processes=2 --multi_gpu --mixed_precision=bf16 script3_train.py
"""
...
MODEL_SHORT_NAME  = "hubert-large"
FEATURES_DIR      = "/workspace/audio-em/features"
STRATEGY          = "lora"           # "frozen" or "lora"
BASE_RESULTS_DIR  = "/workspace/audio-em/finetune-results"

# ── Batch / accumulation ──────────────────────
# HuBERT-Large is ~1GB VRAM per sample at 10s audio.
# With 2×GPU: BATCH_SIZE=2 per GPU, GRAD_ACCUM=8 → effective batch = 2×2×8 = 32
BATCH_SIZE        = 2
GRAD_ACCUM_STEPS  = 8
...
# ── DataLoader ────────────────────────────────
# CRITICAL: keep low to avoid OOM from worker RAM.
# Each worker holds a copy of the Dataset + loaded tensors.
# 2 workers per GPU is safe for most setups.
NUM_WORKERS     = 2

# ── Gradient checkpointing ────────────────────
# Cuts VRAM ~40% at cost of ~20% speed. Recommended when BATCH_SIZE must be small.
USE_GRAD_CKPT   = True
...
# ─────────────────────────────────────────────
# DATASET  (augmentation safe for worker processes)
# ─────────────────────────────────────────────

class EmotionDataset(Dataset):
    """
    Loads pre-extracted waveform tensors from .pt files.

    Augmentation strategy (train only):
      - Gaussian noise         p=0.50  — trivial, no extra memory
      - Time shift ±10%        p=0.40  — torch.roll, in-place
      - Amplitude scaling      p=0.40  — scalar multiply
      NO speed perturbation: torchaudio.Resample allocates ~200MB per call
      inside worker processes → triggers OOM killer. Removed entirely.
    """
    ...
    def __getitem__(self, idx):
        ...
        waveform = data["waveform"]   # (max_samples,)  float32
    ...
    def _augment(self, w: torch.Tensor) -> torch.Tensor:
        # 1. Gaussian noise  (safe: no allocation beyond w.shape)
        if random.random() < 0.50:
            w = w + torch.randn_like(w) * self.noise_std

        # 2. Time shift ±10 %  (torch.roll: O(1) memory)
...
def build_weighted_sampler(dataset: EmotionDataset) -> WeightedRandomSampler:
    ...
    weights = [class_w[r["emotion_id"]] for r in dataset.records]
...
class HuBERTEmotionClassifier(nn.Module):
    def __init__(self, strategy: str, use_grad_ckpt: bool = False):
        ...
        hidden        = self.hubert.config.hidden_size   # 1024 for large

        # Enable gradient checkpointing BEFORE LoRA wrapping
        if use_grad_ckpt:
            self.hubert.gradient_checkpointing_enable()
            print("  [HuBERT] Gradient checkpointing ENABLED  (~40% VRAM saved)")
        ...
        if strategy == "frozen":
            ...
            print("  [HuBERT] Encoder FROZEN — training head only.")
        elif strategy == "lora":
            cfg = LoraConfig(
...
def save_training_curves(history, output_dir, strategy):
    ...
    axes[1, 0].set_title("Overfitting gap (val_loss – train_loss)")
...
def main():
    ...
    if is_main:
        ...
        print(f"\n{'='*70}")
        print(f"  HuBERT Emotion Training  (v2)")
        print(f"{'='*70}")
        ...
        print(f"  Focal γ:        {FOCAL_GAMMA}  |  Label smooth: {LABEL_SMOOTHING}")
        print(f"  LoRA r/α:       {LORA_R}/{LORA_ALPHA}")
        print(f"  Grad ckpt:      {USE_GRAD_CKPT}  |  Workers/loader: {NUM_WORKERS}")
        print(f"{'='*70}\n")
    ...
    # → avoids re-fork overhead (~30s per epoch on large datasets)
    # prefetch_factor=2 (default) is fine; don't raise it — more RAM pressure
    ...
    val_loader = DataLoader(
        val_dataset,
        batch_size=BATCH_SIZE * 2,   # eval can use 2× batch (no gradients)
        shuffle=False,
        num_workers=NUM_WORKERS,
        pin_memory=pin,
        persistent_workers=(NUM_WORKERS > 0),
    ...
    model     = HuBERTEmotionClassifier(STRATEGY, use_grad_ckpt=USE_GRAD_CKPT)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if is_main:
            ...
            print(f"  ETA (remaining epochs): ~{eta_min:.0f} min")
