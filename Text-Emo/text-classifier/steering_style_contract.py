...
pure_steering.py
---------------------
Re-inject emotion vector every single turn.

Pipeline per turn:
    user_sentence
        → Classifier       (emotion, target, user_intensity)
        → StateMemory      (ai_state with blended intensity)
        → DecisionEngine   (vector, vector_intensity, mode, style_contract)
        → EmotionSteerer   (inject vector with effective_alpha = ALPHA × vector_intensity)
        → generate response with style contract constraints (prompt injection)
        → remove steerer
        → next turn repeats
...
Output:
    /data/kaushal/Various-Model/classification/output/{model_name}/multiturn_test/run_{timestamp}/{TARGET_EMOTION}/
"""
...
from decision_engine import DecisionEngine, StyleContract
...
def build_style_prompt_instructions(style_contract: StyleContract) -> str:
    ...
    # Bullets instruction
    if style_contract.allow_bullets:
        bullets_inst = "- use bullet points for listing items"
    else:
        bullets_inst = "- do not use numbered lists"
    
    # Questions instruction
    if style_contract.allow_questions:
        questions_inst = "- ask questions if appropriate"
    else:
        questions_inst = "- do not ask any questions"
    
    # Commands instruction
    if style_contract.allow_commands:
        commands_inst = "- use suggestions"
    else:
        commands_inst = "- do not give commands"
    
    # Profanity instruction (always false in your setup)
    if style_contract.profanity:
        profanity_inst = "- Mild profanity is permitted if natural"
    else:
        profanity_inst = "- do not use any profanity"
    
    return f"""Response instructions:
                - Maximum length: {style_contract.max_words} words
                - Maximum sentences: {style_contract.max_sentences}
                {bullets_inst}
                {questions_inst}
                {commands_inst}
                {profanity_inst}
...
def apply_style_to_generation_kwargs(
...
# ============================================================================
# MULTI-TURN CONVERSATION MANAGER (UPDATED with Style Contract Prompt Injection)
# ============================================================================

class MultiTurnConversation:
    """Full conversation history preserved across all turns. No system prompt."""
    
    # Base generation parameters (can be overridden by style contract)
    BASE_GENERATION_KWARGS = {
        "do_sample": False,
        "eos_token_id": None,  # will be set in __init__
        "pad_token_id": None,  # will be set in __init__
        "use_cache": True,
        "min_new_tokens": 10,
    ...
    def __init__(self, model, tokenizer):
        ...
        # Set token IDs after tokenizer is available
        self.BASE_GENERATION_KWARGS["eos_token_id"] = tokenizer.eos_token_id
        self.BASE_GENERATION_KWARGS["pad_token_id"] = tokenizer.pad_token_id
    ...
    def build_prompt_with_style(self, user_message: str, style_contract: Optional[StyleContract] = None) -> str:
        """
        Build the full prompt including conversation history, user message, and style instructions.
        
        Parameters
        ----------
        user_message : str - the current user input
        style_contract : StyleContract - style rules to inject into prompt
        
        Returns
        -------
        prompt : str - complete prompt ready for tokenization
        """
        # Start with conversation history (excluding the current user message)
        # We need to add the current user message separately with style instructions
        
        # Build conversation history without the current turn
        history_prompt = ""
        ...
        if style_contract is not None:
            style_instructions = build_style_prompt_instructions(style_contract)
            prompt += f"{style_instructions}"        
        
        return prompt

    @torch.no_grad()
    def generate_response(self, user_message: str, style_contract: Optional[StyleContract] = None) -> tuple[str, str]:
        """
        Generate response with style contract constraints applied via prompt injection.
        
        Parameters
        ----------
        user_message : str - the current user input
        style_contract : StyleContract from decision engine
                        Controls max_words, allow_questions, allow_commands, etc.
        
        Returns
        -------
        response : str (raw from model)
        clean_response : str (normalized for history)
        """
        # Build prompt with style instructions injected
        prompt = self.build_prompt_with_style(user_message, style_contract)
        
        # Tokenize
        inputs = self.tokenizer(prompt, return_tensors="pt").to(self.model.device)
        
        # Start with base generation kwargs
        gen_kwargs = self.BASE_GENERATION_KWARGS.copy()
        
        # Apply style contract for token limit if provided
        if style_contract is not None:
            gen_kwargs["max_new_tokens"] = 100
            print(f"         Style: max_words={style_contract.max_words} (prompt guidance only)")
        ...
        # Add to conversation history (only the response, not the prompt with instructions)
...
def run_pathway(
    ...
    Per turn:
        ...
        3. DecisionEngine → vector, vector_intensity, mode, style_contract
        4. If vector != "none":
        ...
        5. Generate with style contract injected into prompt + token limit
        6. Steerer removed, next turn starts fresh
    """
    ...
    print(f"\n   {direction_type.upper()} | {emotion_name} | {total_turns} turns")
    ...
    conversation_id = f"{run_timestamp}_{emotion_name}_{direction_type}_{np.random.randint(1000, 9999)}"

    result = {
        "conversation_id": conversation_id,
        "pathway": "reinject_every_turn",
        "experiment": {
            ...
            "style_contract_enforced": True,
            "style_enforcement_method": "prompt_injection",
        },
        "scenario": {
    ...
    try:
        for turn_idx, turn_entry in enumerate(turns_data):
            ...
            # ── Step 4: Setup Steerer (if needed) ─────────────────────────────
            ...
            # ── Step 5: Generate with Style Contract (Prompt Injection + Token Limit) ──
            response, clean_response = conv.generate_response(
                user_message=user_sentence,
                style_contract=decision.style_contract
            )
            ...
            # Add user message to conversation history AFTER generation?
            # Actually, the conversation manager needs the user message for history
            # Let's add it now
            ...
            turn_record = {
                ...
                "decision": {
                    ...
                    "style_contract": {
                        "max_words":          decision.style_contract.max_words,
                        "max_sentences":      decision.style_contract.max_sentences,
                        "allow_bullets":      decision.style_contract.allow_bullets,
                        "allow_questions":    decision.style_contract.allow_questions,
                        "allow_commands":     decision.style_contract.allow_commands,
                        "profanity":          decision.style_contract.profanity,
                    },                    
                },
            ...
            print(f"         Response length: {len(response.split())} words / {decision.style_contract.max_words} max")
            time.sleep(0.5)
    ...
    # ── Save output ───────────────────────────────────────────────────────────
    out_file = output_dir / f"{conversation_id}.json"
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, ensure_ascii=False)

    # Append to combined JSONL file for this emotion and direction type
...
def main():
    ...
    ALL_EMOTIONS = ["anger", "sadness", "happiness", "fear", "disgust", "surprise"]
    
    print("=" * 80)
    print("MULTI-TURN EMOTION STEERING (RE-INJECT EVERY TURN)")
    print("WITH STYLE CONTRACT ENFORCEMENT (PROMPT INJECTION)")
    print("=" * 80)
    print(f"Model          : {MODEL_PATH}")
    print(f"Base Alpha     : {ALPHA}  (scaled by vector_intensity each turn)")
    print(f"Layers         : {LAYERS}")
    print(f"Emotions       : {ALL_EMOTIONS}")
    print(f"Style Contract : Active (prompt injection + max_words → max_new_tokens)")
    print("=" * 80)
    ...
    # ── Create main run directory: run_{timestamp}/ ───────────────────────────
    run_output_dir = OUTPUT_BASE / f"run_{timestamp}"
    run_output_dir.mkdir(parents=True, exist_ok=True)
    print(f"\n[1/6] Main run directory: {run_output_dir}")
    ...
    master_metadata = {
        ...
        "style_contract": {
            "enabled": True,
            "enforcement_method": "prompt_injection",
            "fields_enforced": ["max_words", "max_sentences", "allow_bullets", "allow_questions", "allow_commands", "profanity"],            
        },
    ...
    with open(run_output_dir / "master_metadata.json", "w") as f:
    ...
    for emotion_idx, target_emotion in enumerate(ALL_EMOTIONS, 1):
        ...
        # ── Create emotion-specific subdirectory: run_{timestamp}/{TARGET_EMOTION}/ ──
        emotion_output_dir = run_output_dir / target_emotion
        ...
        emotion_metadata = {
            ...
            "style_contract_enforced": True,
            "style_enforcement_method": "prompt_template_injection",
            "timestamp":       time.time(),
        }
    ...
    with open(run_output_dir / "all_emotions_summary.json", "w") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)

    print("\n" + "=" * 80)
    print("✅ ALL EMOTIONS COMPLETE!")
    print(f"📁 Master output directory: {run_output_dir}")
    print("=" * 80)
